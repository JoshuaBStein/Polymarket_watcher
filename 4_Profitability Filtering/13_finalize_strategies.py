...
# --- STRATEGY THRESHOLDS ---
THRESH_GOOD = 0.75  # "Sniper" (High Profitability)
THRESH_ELITE = 0.90 # "God Mode" (Almost Certainty)

print(f"--- FINAL PROFITABILITY PIPELINE STARTING ---")

# --- 1. LOAD TRADES ---
try:
    print("Loading Trades...")
    # Load 'market_id' specifically to merge
...
try:
    ...
    # Load 'id' (to match market_id) and 'market_slug' (to track price history)
    ...
    # RENAME 'id' TO 'market_id' to match trades.csv
...
# Using a larger test set (20%) to validate our thresholds properly
...
# Get Probabilities (Confidence Scores) instead of just 0/1
probs = model.predict_proba(X_test)[:, 1]

# --- STRATEGY 1: SNIPER (0.75) ---
preds_good = (probs >= THRESH_GOOD).astype(int)
prec_good = precision_score(y_test, preds_good, zero_division=0)
count_good = preds_good.sum()
acc_good = accuracy_score(y_test, preds_good)

print(f"[1] SNIPER STRATEGY (Threshold > {THRESH_GOOD})")
print(f"    - Win Rate (Precision): {prec_good:.2%}")
print(f"    - Trades Taken:         {count_good:,}")
print(f"    - Overall Accuracy:     {acc_good:.2%}")
print("-" * 50)

# --- STRATEGY 2: GOD MODE (0.90) ---
preds_elite = (probs >= THRESH_ELITE).astype(int)
prec_elite = precision_score(y_test, preds_elite, zero_division=0)
count_elite = preds_elite.sum()
acc_elite = accuracy_score(y_test, preds_elite)

print(f"[2] GOD MODE STRATEGY (Threshold > {THRESH_ELITE})")
print(f"    - Win Rate (Precision): {prec_elite:.2%}")
print(f"    - Trades Taken:         {count_elite:,}")
print(f"    - Overall Accuracy:     {acc_elite:.2%}")
print("=" * 50)

# Save
output_path = os.path.join(BASE_DIR, 'whale_filter_xgb.json')
